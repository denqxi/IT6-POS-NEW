...
function fetchMenuItemsByCategory($conn, $categoryId, $categoryName) {
    ...
    $query = ($categoryId === 'all') ? 
        "SELECT mi.item_id, mi.item_name, ip.item_price, c.category_name 
         FROM menu_Items mi
         JOIN item_Prices ip ON mi.item_id = ip.item_id
         JOIN Categories c ON mi.category_id = c.category_id
         WHERE mi.is_active = 1 AND ip.effective_date <= CURDATE()
         ORDER BY mi.item_id ASC" 
        : 
        "SELECT mi.item_id, mi.item_name, ip.item_price 
         FROM menu_Items mi
         JOIN item_Prices ip ON mi.item_id = ip.item_id
         WHERE mi.is_active = 1 
           AND mi.category_id = $categoryId  
    ...
    <div class="row row-cols-2 row-cols-sm-3 row-cols-md-4 row-cols-lg-6 g-3 pb-5" id="menu-items"
        ...
        <?php if ($result && $result->num_rows > 0): ?>
            ...
            <div class="card text-center shadow-sm m-2 position-relative"
                ...
                <!-- Product Image -->
                <img src="./assets/menuitems/<?php echo $row['item_id']; ?>.png" 
                     class="card-img-top" 
